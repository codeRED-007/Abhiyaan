...
private:
    ...
    void midpoint_publisher(vector<cv::Point> largestCluster, vector<cv::Point> secondLargestCluster, cv::Mat lanes_binary) {
        ...
        if (midpoint_flag_farthest) {
            ...
            if (cam_info_received) {
                sensor_msgs::msg::PointCloud2 cloud_far = process_point(avg_farthest.y + lanes_binary.rows / 2, avg_farthest.x);
        ...
        if (midpoint_flag_closest) {
            ...
            if (cam_info_received) {
                sensor_msgs::msg::PointCloud2 cloud_near = process_point(avg_closest.y + lanes_binary.rows / 2, avg_closest.x);
                cam_info_received = false;
 ...
 sensor_msgs::msg::PointCloud2 process_point(int y, int x) {
    ...
    auto matmul = [](double *a, double *b, double *c) {
        for (int i = 0; i < 3; ++i) {
            double temp = 0;
            for (int k = 0; k < 3; ++k) {
                temp += a[i * 3 + k] * b[k];
    ...
    auto dot = [](double* a, double* b, double* c, int rows) {
    ...
    if (caminfo.size() != 9) {
        throw std::runtime_error("camera_info.k does not have 9 elements");
    }

    Eigen::Map<Matrix<double, 3, 3, RowMajor>> mat(caminfo.data());
    mat = mat.inverse();
    vector<double> inv_caminfo = caminfo.data(); // K-1 (inverse of K)

    vector<double> kin_uv(3), denom(1);
    // double *d_uv, *d_caminfo, *d_kin_uv, *d_denom, *d_nc;

    // Device allocations
    // if (cudaMalloc((void**)&d_kin_uv, sizeof(double) * 3) != cudaSuccess ||
    //     cudaMalloc((void**)&d_caminfo, sizeof(double) * 9) != cudaSuccess ||
    //     cudaMalloc((void**)&d_denom, sizeof(double)) != cudaSuccess ||
    //     cudaMalloc((void**)&d_nc, sizeof(double) * 3) != cudaSuccess ||
    //     cudaMalloc((void**)&d_uv, sizeof(double) * 3) != cudaSuccess) {
    //     throw std::runtime_error("CUDA memory allocation failed");
    // }

    // Prepare UV homogeneous coordinates
    std::vector<double> uv_hom = { static_cast<double>(x), static_cast<double>(y), 1.0 };


    // Copy to device
    // if (cudaMemcpy(d_caminfo, inv_caminfo, sizeof(double) * 9, cudaMemcpyHostToDevice) != cudaSuccess ||
    //     cudaMemcpy(d_uv, uv_hom, sizeof(double) * 3, cudaMemcpyHostToDevice) != cudaSuccess ||
    //     cudaMemcpy(d_nc, uv.data(), sizeof(double) * 3, cudaMemcpyHostToDevice) != cudaSuccess) {
    //     throw std::runtime_error("CUDA memory copy failed");
    // }

    // Launch matrix multiplication // K-1 x [x ,y, 1]
    matmul(inv_caminfo.data(), uv_hom.data(),kin_uv.data())
    // Check for any kernel launch errors
    
 
    // if (cudaMemcpy(kin_uv.data(), d_kin_uv, sizeof(double) * 3, cudaMemcpyDeviceToHost) != cudaSuccess) {
    ...
    if (cudaMemcpy(denom.data(), d_denom, sizeof(double) * 1, cudaMemcpyDeviceToHost) != cudaSuccess) {
        throw std::runtime_error("CUDA memory copy back failed");
    }
    cout<<"nigga12"<<endl;
