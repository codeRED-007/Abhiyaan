...
class LaneDetector(Node):
    ...
    def get_binary(self, data):
        ...
        #binary_image[:240, :] = 0 # Setting top half of the image as black.
        # binary_image = cv2.medianBlur(binary_image, 9)
        # cv2.imshow("median",binary_image)
        # cv2.waitKey(1)
    ...
    def get_world_coordinates(self,binary_image):
        ...
        cg = CameraGeometry(image_height=binary_array.shape[0],image_width=binary_array.shape[1])
        binary_coordinates = np.column_stack(np.where(binary_array == 255))
        binary_coordinates = binary_coordinates[::10]
