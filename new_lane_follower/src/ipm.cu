...
class IPM : public rclcpp::Node
{
  ...
  private:
	...
	// cv::inRange(gray_image, cv::Scalar(200), cv::Scalar(240), gray_image); 
	...
	// //what does this do?
	matmul(k.data(), nor.data(), uv.data()); // UV is Nc (normal in Camera Frame
	...
	mat = mat.inverse();
	double *inv_caminfo = mat.data(); // K-1 Matrix
	...
	cudaMalloc((void **) &d_uv_hom, sizeof(double)*3*rows);
	cudaMalloc((void **) &d_kin_uv, sizeof(double)*3*rows);
	...
	cudaMemcpy(d_uv_hom, uv_hom.data(), sizeof(double)*3*rows, cudaMemcpyHostToDevice); // Input Data
	cudaMemcpy(d_uv, uv.data(), sizeof(double)*3, cudaMemcpyHostToDevice); // Nc ( normal in camera frame )

	//batch multiplication
	//launching rows no of threads and one block
	dev_matmul<<<BLOCKS, (rows+BLOCKS-1)/BLOCKS>>>(d_caminfo, d_uv_hom, d_kin_uv, rows); // K-1 x [x,y,1]
	dot<<<BLOCKS, (rows+BLOCKS-1)/BLOCKS>>>(d_uv, d_kin_uv, d_denom, rows); // Nc x K-1 x [x,y,1]
	...
	for(int i=0; i < rows; ++i){
	 ...
	 pub_pointcloud.header.frame_id = "ipm_frame";
